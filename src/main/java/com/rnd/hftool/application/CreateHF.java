...
public class CreateHF
{
    ...
    private void getPatchFile()
    {
        ...
        if (isNotEmpty(files))
        {
            patchFile = Arrays.stream(files).sorted((o1, o2) -> (int) (o2.lastModified() - o1.lastModified())).findFirst().orElse(null);
        }
    ...
    private void treatPatchFileRecords()
    {
        ...
        parsedPatchFileRecords.forEach(record -> treatedPatchFileRecords
                .add(setUnixPathSeparator(replace(replace(record, SOURCE_JAVA_PACKAGE, TARGET_CLASSES_PACKAGE), EXTENSION_JAVA, EXTENSION_CLASS)).trim()));
        ...
        moduleWiseJarRecordDTOSMap.keySet().forEach(moduleName ->
                                                    {
                                                        ...
                                                        {
                                                            String jarPath = componentPath + finalLocalHotfixDirectoryName + UNIX_SEPARATOR + (isNotBlank(
                                                                    moduleName)
                                                                                                                                               ? replace(
                                                                    moduleName, UNIX_SEPARATOR, UNDERSCORE) + UNDERSCORE
                                                                                                                                               : EMPTY) + suffix + EXTENSION_JAR;
                                                            try
                                                            {
